...
export function CaseStudySectionV2() {
  ...
  return (
    <section id="case-studies" className="relative w-full bg-[#F5F5F7] text-[#1D1D1F] py-16 md:py-32">
      <div className="max-w-[1400px] mx-auto w-full px-6 md:px-12">
        
        {/* Standard Global Header Row */}
        <div className="border-t border-[#1D1D1F]/10 pt-6 mb-16 flex flex-col md:flex-row justify-between items-start gap-6">
          ...
          <div className="flex flex-col gap-4 max-w-[600px]">
            ...
            <p className="text-[14px] md:text-[15px] font-medium text-[#1D1D1F]/70 leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {cases.map((item, idx) => (
            <div 
              key={idx} 
              className="group bg-white rounded-[8px] border border-[#1D1D1F]/10 overflow-hidden flex flex-col shadow-[0_2px_15px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] transition-all duration-300"
            >
              {/* Image Section */}
              <div className="w-full h-[280px] md:h-[240px] relative overflow-hidden bg-[#F5F5F7]">
                <div 
              ...
              <div className="p-8 flex flex-col flex-1">
                <span className="text-[11px] font-medium tracking-wider uppercase text-[#1D1D1F]/60 block mb-3">
                  {item.num} — {item.category}
                </span>
                
                <h3 className="text-[20px] md:text-[22px] font-normal text-[#1D1D1F] leading-[1.3] mb-4">
                  {item.title}
                </h3>
                
                <p className="text-[14px] text-[#1D1D1F]/70 leading-relaxed font-normal mb-8 flex-1">
                  {item.desc}
                </p>

                {/* Strict Design System CTA (4px radius) */}
                <button className="self-start px-6 py-2.5 rounded-[4px] border border-[#1D1D1F]/20 text-[#1D1D1F] text-[13px] font-medium hover:bg-[#1D1D1F] hover:text-white transition-colors flex items-center gap-2">
                  View Case Study
