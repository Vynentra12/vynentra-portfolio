...
export function ProcessSectionV2() {
  ...
  return (
    <section id="process" className="relative w-full bg-[#F5F5F7] text-[#1D1D1F] py-16 md:py-32 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 relative">
          ...
          <div className="hidden lg:block lg:col-span-5 relative">
            <div className="sticky top-32">
              <p className="text-[#1D1D1F]/70 text-[14px] md:text-[15px] leading-relaxed max-w-[340px] font-medium mb-12">
              ...
              <div className="relative opacity-[0.03] pointer-events-none w-[500px] h-[500px] left-[-80px]">
                <svg viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg" className="w-full h-full stroke-black fill-none" strokeWidth="0.75">
                  <path d="M 0,350 Q 150,250 250,350 T 500,300 L 500,500 L 0,500 Z" stroke="none" fill="#000000" fillOpacity="0.02" />
          ...
          <div className="lg:col-span-7 relative z-10 pt-4 lg:pt-0">
            ...
            <div className="absolute left-[52px] top-[4rem] bottom-[4rem] w-[1px] bg-[#1D1D1F]/10 hidden md:block z-0"></div>

            <div className="flex flex-col gap-3 md:gap-4">
              {steps.map((step, index) => (
                <div 
                  key={index} 
                  className="bg-white rounded-[16px] p-5 md:p-6 shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-[#1D1D1F]/5 flex flex-col md:flex-row items-start gap-4 md:gap-8 relative z-10 group hover:-translate-y-1 transition-transform duration-300"
                >
                  {/* Icon Circle */}
                  <div className="w-[56px] h-[56px] rounded-full bg-white shadow-[0_4px_15px_rgb(0,0,0,0.05)] border border-[#1D1D1F]/5 flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform duration-500">
                    <step.icon className="w-6 h-6 text-[#1D1D1F]/70" strokeWidth={1.25} />
                  </div>

                  {/* Content Container */}
                  <div className="flex flex-col md:flex-row items-start gap-4 md:gap-8 w-full pt-1 md:pt-2">
                    {/* Big Number */}
                    <div className="text-[28px] md:text-[36px] font-medium text-[#1D1D1F]/15 leading-none shrink-0 w-[44px]">
                      {step.num}
                    </div>

                    {/* Text Content */}
                    <div className="flex flex-col pt-1">
                      <h3 className="text-lg md:text-[20px] font-bold text-[#1D1D1F] mb-1.5 leading-tight">
                        {step.title}
                      </h3>
                      <p className="text-[13px] md:text-[14px] font-semibold text-[#1D1D1F]/80 mb-2">
                        {step.subtitle}
                      </p>
                      <p className="text-[13px] text-[#1D1D1F]/60 leading-relaxed font-medium">
                        {step.desc}
                      </p>
                    </div>
                  </div>
